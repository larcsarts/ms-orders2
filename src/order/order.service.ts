...
const webhookUri =
  'https://hooks.slack.com/services/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';
...
export class OrderService {
  ...
  async getAllOrders(params: OrdersDTO) {
    ...
    const ordersId = orders.map(order => ({ order_id: order.id }));

    const ordersExecuted = await this.executedOrdersRepository.find({
      where: ordersId,
    });
    ...
    const getPercentage = orderExecuted => {
      return math.multiply(
        math.divide(orderExecuted.amount_executed, orderExecuted.order_amount),
        100,
      );
    };
    // @todo check if can be do better
    const data = orders.map(order => ({
      ...
      pair: order.pair,
      executed_orders: ordersExecuted
        .filter(({ order_id }) => order_id === order.id)
        .map(orderExecuted => {
          return {
            identificator: orderExecuted.identificator,
            time_executed: orderExecuted.time_executed,
            amount_executed: orderExecuted.amount_executed,
            price_unity: orderExecuted.price_unity,
            total: orderExecuted.total,
            pair: orderExecuted.pair,
            percentage: getPercentage(orderExecuted),
          };
        }),
      total_executed: ordersExecuted
        .filter(({ order_id }) => order_id === order.id)
        .reduce((sum, currentValue) => sum + currentValue.total, 0),
      state: state(order),
    }));
  ...
  async sendDeposit(data: SendDepositDTO) {
    const { user } = await this.clientProxy
      ...
      .catch(err => {
        throw new RpcException(
          'error on get_user_info on orderService.sendDeposit',
        );
      });
    ...
    try {
      ...
      throw new RpcException('cant validate the coin on OrderService.deposit');
    }
  ...
  async checkOrderMarketIsDone(identificator) {
    setTimeout(async () => {
      const order = await this.orderRepository.findOne({
        ...
        slack.webhook(
          {
            ...
            username: 'webhookbot',
            icon_emoji: ':warning:',
            text: `Order with identificator:${identificator} is market and not done in 5 seconds`,
          },
          (err, response) => {
            if (err)
              throw new RpcException(
                'error on send message in slack on orderService.checkOrderMarketIsDone',
              );
            Logger.log(
              `send message in slack because orderId:${identificator} is market and not done in 5 seconds`,
              'orderService.checkOrderMarketIsDone',
              true,
            );
          },
        );
  ...
  async placeOrder(data: PlaceOrderDTO) {
    ...
    try {
      ...
      Logger.log(
        `ordem do usuário #${data.userId} pair dont available`,
        'OrdersService.placeOrder',
        true,
      );
      throw new RpcException('pair don`t available');
    }
    ...
    try {
      ...
      Logger.log(
        `ordem do usuário #${data.userId} coin dont available`,
        'OrdersService.placeOrder',
        true,
      );
      throw new RpcException('pair don`t available 2');
    }
    ...
    if (!user) {
      Logger.log(
        `usuário #${data.userId} nao encontrado`,
        'OrdersService.placeOrder',
        true,
      );
    ...
    if (user.blocked === 1) {
      Logger.log(
        `usuário #${data.userId} tem a conta bloqueada`,
        'OrdersService.placeOrder',
        true,
      );
      throw new RpcException('Conta bloqueada');
    ...
    if (obj.total < 5) {
      Logger.log(
        `usuário #${data.userId} tentou comprar abaixo de 5 reais.`,
        'OrdersService.placeOrder',
        true,
      );
    ...
    obj.side = data.order_type.split(' ')[0];
    obj.operation_type = data.order_type.split(' ')[1];
    obj.operation_mode = data.order_type.split(' ')[2];
  ...
  private async buyOperation(data: PlaceOrderDTO, obj, user: User) {
    Logger.log(
      `start with data:${JSON.stringify(data)}, obj:${JSON.stringify(obj)}`,
    ...
    const balanceResponse = await this.clientProxy
      ...
      .toPromise()
      .catch(err => {
        Logger.log('cant get user balance', 'OrderService.buyOperation', true);
        throw new RpcException(err.message);
      });
    ...
    if (balanceResponse.current_balance < obj.total) {
      Logger.log(`fundos insuficientes`, 'OrderService.buyOperation', true);
    ...
    try {
      const orderSaved = await this.orderRepository.create({
        ...
        operation_mode: obj.operation_mode,
      });
      ...
      const newOrder = {
        ...
        price: orderSaved.price_unity.toString(),
        operation_type: orderSaved.operation_type,
      };
      if (orderSaved.operation_type === 'market')
        this.checkOrderMarketIsDone(orderSaved.identificator);
      return {
        success: true,
  ...
  private async sellOperation(data: PlaceOrderDTO, obj, user: User) {
    const balanceResponse = await this.clientProxy
      ...
      .toPromise()
      .catch(err => {
        throw new RpcException('cant get the balance response');
      });
  ...
  async deleteOpenedOrders(userId: string) {
    const orders = await this.orderRepository.find({
  ...
  async getOrder(data: GetOrderDto) {
    ...
    if (!order) {
      throw new RpcException('Ordem não encontrada');
    }
